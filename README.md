...
| Layer | When it fires | Works on |
|---|---|---|
| `UNNotification`, `.timeSensitive` | app not frontmost | iOS 16+ |
| `AlarmSound` — a looping tone the app plays itself | app frontmost | iOS 16+ |
| `AlarmKit` — a real system alarm | either | iOS 26.1+ |

The middle one closes a gap rather than adding a feature: the app scheduled a
notification for the end of the run, and a notification is not presented while
the app is frontmost. Have the app open and the alarm screen appeared in
complete silence. There was no audio code in the iOS app at all.

`AlarmSound` uses the `.playback` audio session category, so it is audible with
the ring/silent switch set to silent — which is what an alarm is for, and why it
stops the instant the run is acknowledged. `.duckOthers` lowers music instead of
ending it. The tone is generated by `tools/make_alarm_sound.py` from sine waves,
so it carries no third-party rights, same as the beer artwork. It is a seamless
1.6 s loop: both the generator and `AlarmTests` assert the first and last samples
are at silence, because a loop cut mid-cycle clicks on every repeat.

`SystemAlarm` wraps AlarmKit, which since iOS 26 lets a third-party app schedule
an alarm that rings through silent mode and Focus and keeps ringing until it is
dismissed — the behaviour the Clock app has always had and no other app could
reach. The system owns the countdown, so it survives the app being suspended or
killed.

It requires **iOS 26.1**, not 26.0. `AlarmPresentation.Alert`'s 26.0 initialiser
takes a `stopButton` that 26.1 deprecates as "no longer used", and calling the
deprecated form to reach one point release would build a warning into a project
that has none. Below 26.1 the notification still works.

The notification is scheduled either way, even when AlarmKit succeeds. Two alerts
is the failure mode to prefer over none, and stopping a run cancels both.

AlarmKit needs the user's permission and an `NSAlarmKitUsageDescription` in
Info.plist. That string is localized through `BeerChiller/InfoPlist.xcstrings` —
otherwise the system prompt would be the one English sentence in an app that is
